...
public class Dijkstra {
    ...
    private static void hallarCaminoSecuencia(){ //Halla el camino más corto y lo almacena como secuencia de vertices
        ...
        nodosValidos[0]=true;
        for (int i = nodoInicio; i < numVertices; i++) {
    ...
    public static String obtenerCaminoMinimo(int nvert, int inicio, int fin, int pesos[][]){//Devuelve el camino mínimo
        String solucion="";
        int nodo=0;
        numVertices=nvert;
        matrizPesos=pesos;
        ...
        nodo=fin;
        solucion+=(char)(nodo+65);
        while (nodo!=inicio) {
            nodo = nodosSecuencia[nodo];
            solucion=(char)(nodo+65)+solucion;
        }
        return solucion;
    }
}
